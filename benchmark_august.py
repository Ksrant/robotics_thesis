...
PUSH_MODULE = "push_camera"   # must be in the same folder as this benchmark.py
try:
...
# ── Positions — STRICTEMENT alignées sur la Table 6.1 du mémoire ──────────
#   A, B : grille de benchmark (obliquités ~27° et ~11°)
#   C, D : géométries d'ablation  (obliquités ~14° et ~37°)
POS_A = {"name": "A", "obj": [0.35, 0.10], "target": [0.55, 0.20]}   # ~27°
POS_B = {"name": "B", "obj": [0.30, 0.10], "target": [0.55, 0.05]}   # ~11°
POS_C = {"name": "C", "obj": [0.35, 0.05], "target": [0.55, 0.00]}   # ~14°
POS_D = {"name": "D", "obj": [0.35, 0.05], "target": [0.55, 0.20]}   # ~37°

GRID_FULL = {
    "shapes":    ["cube", "cylinder"],
    "masses":    [0.5, 1.0, 1.5, 2.0],       # §6.1 annonce ces quatre-là
...
GRID_ABLATION = {
    ...
    "masses":    [0.25],
    "frictions": [0.3],
    "positions": [POS_C, POS_D],
}

GRID_NOISE = {
    "shapes":    ["cube", "cylinder"],
    "masses":    [0.5],
    "frictions": [0.3],                        # §6.5 dit µ=0.3, pas 0.5
    "positions": [POS_A],
}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--traj-view", type=str, default=None,
                        metavar="XMIN,XMAX,YMIN,YMAX",
                        help="Force identical axis limits on every trajectory "
                             "plot of this campaign (for side-by-side panels).")   
    parser.add_argument("--occlusion-grid", action="store_true",
                        help="8-run sub-grid for the occlusion sweep: 2 shapes, "
                             "2 masses, mu=0.3, positions A and B.")                 
    args = parser.parse_args()
    if args.traj_view:
        xa, xb, ya, yb = [float(v) for v in args.traj_view.split(",")]
        push_mod.TRAJ_VIEW_XLIM = (xa, xb)
        push_mod.TRAJ_VIEW_YLIM = (ya, yb)
        print(f"[benchmark] Trajectory view forced to x{(xa, xb)} y{(ya, yb)}")
    ...
    if args.analyse:
        ...
        grid = (GRID_OCCLUSION if args.occlusion_grid
            else GRID_NOISE if args.noise_grid 
            else GRID_QUICK if args.quick 
            else GRID_FULL)
        all_rows = []
        ...
        grid = (GRID_OCCLUSION if args.occlusion_grid
            else GRID_ABLATION if args.ablation_grid 
            else GRID_QUICK if args.quick 
            else GRID_FULL)
        camera_kwargs = None
        if args.camera:
