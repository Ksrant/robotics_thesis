...
ROBOT_BASE_XY  = np.array([-0.10, 0.0])   # position XY de la base Panda dans le monde
FRANKA_MAX_REACH_XY = 0.80   
SPHERE_RADIUS     = 0.05
TABLE_TOP_Z       = 0.050
...
DEFAULT_OBJECT_POS    = np.array([0.15, 0.05, 0.075])
DEFAULT_OBJECT_TARGET = np.array([0.30, 0.40])
...
def hover_point_xy(object_pos_xy, fn, half_extent):
    """
    Point de survol = côté approche + offset de sécurité.
    Réduit l'offset progressivement si le point est hors portée du Franka.
    Dernier recours : pointer depuis l'objet vers la base robot.
    """
    for offset in [0.12, 0.08, 0.05, 0.02]:
        p = object_pos_xy + (half_extent + SPHERE_RADIUS + offset) * fn
        if np.linalg.norm(p - ROBOT_BASE_XY) <= FRANKA_MAX_REACH_XY:
            return p
 
    # Hors portée dans toutes les directions de fn : aller vers la base
    # en restant à la distance minimale de l'objet
    dir_to_base = ROBOT_BASE_XY - object_pos_xy
    dist_to_base = np.linalg.norm(dir_to_base) + 1e-9
    safe_dir     = dir_to_base / dist_to_base
    return object_pos_xy + (half_extent + SPHERE_RADIUS + 0.05) * safe_dir
...
class PushController(LeafSystem):
    ...
    def _plan_reposition(self, object_pos_xy, p_sph_current):
        ...
        # FIX B : retrait clampé à la portée
        retrait_raw = object_pos_xy + (self.half_extent + RETRAIT) * fn
        if np.linalg.norm(retrait_raw - ROBOT_BASE_XY) > FRANKA_MAX_REACH_XY:
            # Réduire RETRAIT jusqu'à rentrer dans l'espace de travail
            for r in [0.08, 0.06, 0.04]:
                retrait_raw = object_pos_xy + (self.half_extent + r) * fn
                if np.linalg.norm(retrait_raw - ROBOT_BASE_XY) <= FRANKA_MAX_REACH_XY:
                    break
        retrait_xy = retrait_raw
 
        # hover_point_xy est maintenant workspace-aware (FIX B)
        p_hover   = hover_point_xy(object_pos_xy, fn, self.half_extent)
        p_contact = contact_point_xy(object_pos_xy, fn, self.half_extent)
        ...
        for i, wp in enumerate(self._waypoints):
            reach = np.linalg.norm(wp[:2] - ROBOT_BASE_XY)
            print(f"  WP{i}: {np.round(wp,3)}  reach={reach:.3f}m "
                  f"{'⚠ hors portée' if reach > FRANKA_MAX_REACH_XY else '✓'}")
...
if __name__ == "__main__":
    SHAPE    = "cylinder"
    SDF_PATH = "models_generated/envcylinder_test.sdf"
    HALF_EXT = 0.05

    metrics = run_trial(
        sdf_path=SDF_PATH, shape_type=SHAPE, half_extent=HALF_EXT,
        object_model_name="object", sim_time=30.0, render=True,
    )
    print("\n=== Résultat ===")
